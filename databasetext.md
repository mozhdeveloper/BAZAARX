...
CREATE TABLE public._orphan_orders_audit_20260424 (
...
CREATE TABLE public.admin_settings (
  id text NOT NULL DEFAULT 'global'::text CHECK (id = 'global'::text),
...
CREATE TABLE public.courier_rate_cache (
  ...
  service_type text NOT NULL DEFAULT 'standard'::character varying,
...
CREATE TABLE public.delivery_bookings (
  ...
  buyer_id uuid NOT NULL,
  courier_code text NOT NULL CHECK (courier_code = ANY (ARRAY['jnt'::character varying::text, 'lbc'::character varying::text, 'flash'::character varying::text, 'ninjavan'::character varying::text, 'grabexpress'::character varying::text, 'lalamove'::character varying::text])),
...
CREATE TABLE public.inventory_movements (
  ...
  reason text NOT NULL CHECK (reason = ANY (ARRAY['order'::text, 'order_cancel'::text, 'restock'::text, 'adjustment'::text, 'return'::text, 'pos_sale'::text, 'pos_refund'::text, 'manual'::text])),
  order_id uuid,
  actor_id uuid,
  notes text,
...
CREATE TABLE public.order_shipments (
  ...
  CONSTRAINT order_shipments_seller_id_fkey FOREIGN KEY (seller_id) REFERENCES public.sellers(id)
);
...
CREATE TABLE public.payment_transactions (
  ...
  buyer_id uuid NOT NULL,
  seller_id uuid NOT NULL,
  gateway text NOT NULL DEFAULT 'paymongo'::character varying,
  gateway_payment_intent_id text,
  gateway_payment_method_id text,
  gateway_source_id text,
  gateway_checkout_url text,
  amount numeric NOT NULL,
  currency text NOT NULL DEFAULT 'PHP'::character varying,
  payment_type text NOT NULL CHECK (payment_type = ANY (ARRAY['card'::character varying::text, 'gcash'::character varying::text, 'maya'::character varying::text, 'grab_pay'::character varying::text, 'bank_transfer'::character varying::text, 'cod'::character varying::text])),
  status text NOT NULL DEFAULT 'pending'::character varying CHECK (status = ANY (ARRAY['pending'::character varying::text, 'awaiting_payment'::character varying::text, 'processing'::character varying::text, 'paid'::character varying::text, 'failed'::character varying::text, 'cancelled'::character varying::text, 'refunded'::character varying::text, 'partially_refunded'::character varying::text])),
  description text,
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  escrow_status text NOT NULL DEFAULT 'none'::character varying CHECK (escrow_status = ANY (ARRAY['none'::character varying::text, 'held'::character varying::text, 'released'::character varying::text, 'refunded'::character varying::text])),
...
CREATE TABLE public.refund_return_periods (
  ...
  resolution_source text CHECK (resolution_source IS NULL OR (resolution_source = ANY (ARRAY['buyer'::text, 'seller'::text, 'admin'::text, 'system'::text, 'auto'::text, 'auto-escalation'::text]))),
...
CREATE TABLE public.return_messages (
  ...
  sender_id uuid,
  sender_role text NOT NULL CHECK (sender_role = ANY (ARRAY['buyer'::text, 'seller'::text, 'admin'::text, 'system'::text])),
  body text NOT NULL,
  attachments ARRAY DEFAULT '{}'::text[],
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT return_messages_pkey PRIMARY KEY (id),
  CONSTRAINT return_messages_return_id_fkey FOREIGN KEY (return_id) REFERENCES public.refund_return_periods(id),
  CONSTRAINT return_messages_sender_id_fkey FOREIGN KEY (sender_id) REFERENCES auth.users(id)
);
...
CREATE TABLE public.seller_payout_settings (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  seller_id uuid NOT NULL UNIQUE,
  payout_method text NOT NULL DEFAULT 'bank_transfer'::character varying CHECK (payout_method = ANY (ARRAY['bank_transfer'::character varying::text, 'gcash'::character varying::text, 'maya'::character varying::text])),
  bank_name text,
  bank_account_name text,
  bank_account_number text,
  ewallet_provider text CHECK (ewallet_provider = ANY (ARRAY['gcash'::character varying::text, 'maya'::character varying::text])),
  ewallet_number text,
...
CREATE TABLE public.seller_payouts (
  ...
  payout_method text NOT NULL CHECK (payout_method = ANY (ARRAY['bank_transfer'::character varying::text, 'gcash'::character varying::text, 'maya'::character varying::text])),
  payout_account_details jsonb NOT NULL DEFAULT '{}'::jsonb,
  status text NOT NULL DEFAULT 'pending'::character varying CHECK (status = ANY (ARRAY['pending'::character varying::text, 'processing'::character varying::text, 'completed'::character varying::text, 'failed'::character varying::text, 'on_hold'::character varying::text])),
