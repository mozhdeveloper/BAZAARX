...
## 6. Order Filter Refactoring

Refactored the filtering experience in the Orders tab to declutter the UI and provide a more scalable filtering interface.

### Component Architecture
-   **Renamed Component**: `OrderFilters` → `OrderFilterModal` to better reflect its behavior.
-   **Location**: `src/components/seller/orders/OrderFilterModal.tsx`.
-   **Props Interface**: Updated to accept `visible` and `onClose` alongside filter state/callbacks.

### UI Improvements (`orders.tsx`)
-   **Filter Entry Point**: Replaced the bulky inline tab/buttons with a clean **Filter Button** integrated directly into the Search Bar.
-   **Badge System**: Added a notification badge on the Filter button showing the count of active filters (e.g., "Pending" + "POS" = 2).
-   **Modal Interface**:
    -   **Chips Selection**: Status and Channel options are rendered as selectable chips for better touch targets.
    -   **Real-time Feedback**: Badges on chips show the count of orders for each category.
    -   **Mobile First**: Modal design provides more breathing room than horizontal scroll bars.
-   **Active Filter Summary**: Added a summary row that appears when filters are active, showing exactly what is selected and offering a one-tap "Clear All" action.

### User Experience
-   **Cleaner Header**: Removes visual noise from the main order list view.
-   **Scalability**: The modal approach allows adding more filter types (Date Range, Price, etc.) in the future without breaking the layout.
