/**
 * Delivery Service — Philippine Courier Integration (Mobile)
 *
 ...
 *
 * Sandbox-first: simulates courier API responses without real keys.
 * Uses process.env.EXPO_PUBLIC_* for configuration.
 */
...
const IS_SANDBOX = (process.env.EXPO_PUBLIC_DELIVERY_SANDBOX ?? 'true') === 'true';
...
export class DeliveryService {
  ...
  private getRegion(city: string): string {
    ...
    return REGION_MAP[normalized] || 'luzon';
  }
  ...
  private getEstimatedDays(courier: CourierCode, service: CourierServiceType, sameRegion: boolean): string {
    ...
    return sameRegion ? '2-3 days' : '3-7 days';
  }
