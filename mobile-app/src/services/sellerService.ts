...
export class SellerService {
     ...
     * — the legacy `seller_payout_accounts` is a view and does not support
     * UPSERT/ON CONFLICT (migration 040).
     */
        ...
        try {
            const { error } = await supabase
                ...
                .upsert({
                    ...
                    bank_account_name: payoutData.account_name,
                    bank_account_number: payoutData.account_number,
                    updated_at: new Date().toISOString(),
                }, {
                    onConflict: 'seller_id',
                });
