...
import { X, Gift, PlusCircle, Heart } from 'lucide-react-native';
...
export const AddToWishlistModal = ({ visible, onClose, product }: AddToWishlistModalProps) => {
    ...
    const handleAddToCategory = (categoryId: string) => {
        ...
        if (alreadyInFolder) {
            Alert.alert(LABELS.ALREADY_ADDED_TO_WISHLIST, 'This product is already in that wishlist folder.');
            return;
        }

        const folderName = categories.find((cat) => cat.id === categoryId)?.name || LABELS.WISHLIST;
        addItem(product, 'medium', 1, categoryId);
        Alert.alert(LABELS.ADDED_TO_WISHLIST, `Successfully added to "${folderName}".`);
        handleCloseInternal();
    };
                ...
                <Animated.View style={[
                    ...
                    <View style={styles.header}>
                        <Text style={styles.title}>
                            {isCreatingList ? LABELS.CREATE_NEW_WISHLIST : LABELS.SAVE_TO_WISHLIST}
                        </Text>
                    ...
                    {isCreatingList ? (
                        ...
                        <View style={styles.listContainer}>
                            ...
                            >
                                ...
                                <Text style={styles.createNewText}>{LABELS.CREATE_NEW_WISHLIST}</Text>
                            </Pressable>
                        </View>
                    )}
                </Animated.View>
