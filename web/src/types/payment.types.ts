...
/** PayMongo Payment Intent — server-side object tracking a payment */
export interface PaymentIntent {
  ...
  attributes: {
    ...
    client_key: string;       // for frontend to attach payment method
    last_payment_error?: {
      failed_code: string;
...
/** PayMongo Payment Method — buyer's payment instrument */
...
/** PayMongo Source — for redirect-based payments (GCash, Maya, GrabPay) */
...
/** Payment result returned to the buyer */
export interface PaymentResult {
  success: boolean;
  transactionId: string;
  status: PaymentTransactionStatus;
  gatewayIntentId?: string;
  checkoutUrl?: string;        // for GCash/Maya/GrabPay redirect
  redirectUrl?: string;        // alias for checkoutUrl — used by checkout flow
  clientKey?: string;          // for card 3DS authentication
  referenceNumber?: string;    // for bank transfer
  error?: string;
}

/** Payment transaction DB record */
