...
export default function SellerMessages() {
  ...
  const [buyerPresence, setBuyerPresence] = useState<Map<string, boolean>>(new Map());
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      chatService.trackUserActivity(seller.id);
  ...
  useEffect(() => {
    ...
    dbConversations.forEach((conv) => {
      if (!conv.buyer_id) return;
      const unsub = presenceService.subscribeToPresence(conv.buyer_id, (presence) => {
        setBuyerPresence((prev) => {
          const newMap = new Map(prev);
          newMap.set(conv.buyer_id, presence?.is_online || false);
          return newMap;
        });
      });
  ...
  const normalizedDbConversations: Conversation[] = useMemo(() => {
  ...
  const isHeaderOnline = useRealData && activeConversation?.buyerId 
    ? buyerPresence.get(activeConversation.buyerId) 
    ...
    const updatedConversations = conversations.map(c => {
      if (c.id === selectedConversation) {
        return {
          ...
          messages: [
            ...c.messages,
            { id: `m${Date.now()}`, senderId: 'seller', text: messageText, images: imageUrls, timestamp: new Date(), isRead: true }
          ]
        };
      }
      return c;
  ...
  const filteredConversations = useRealData
    ? normalizedDbConversations.filter(conv => conv.buyerName.toLowerCase().includes(searchQuery.toLowerCase()) || conv.lastMessage.toLowerCase().includes(searchQuery.toLowerCase()))
    : conversations.filter(conv => conv.buyerName.toLowerCase().includes(searchQuery.toLowerCase()) || conv.lastMessage.toLowerCase().includes(searchQuery.toLowerCase())).sort((a, b) => {
        ...
        <div className="flex-1 flex h-full overflow-hidden max-w-7xl mx-auto w-full p-0 md:px-8 md:py-6 gap-0 md:gap-6">
          <div className="w-full md:w-80 border-r bg-white flex flex-col md:rounded-xl md:shadow-[0_8px_30px_rgba(0,0,0,0.04)] md:border border-orange-100 overflow-hidden hidden md:flex">
            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                <div className="flex flex-col items-center justify-center h-64 text-gray-400"><Loader2 className="h-8 w-8 animate-spin mb-2" /><p className="text-sm">Loading conversations...</p></div>
              ) : filteredConversations.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-gray-400"><MessageCircle className="h-12 w-12 mb-2" /><p className="text-sm">No conversations yet</p><p className="text-xs text-gray-400 mt-1">Your customer chats will appear here</p></div>
              ) : (
                filteredConversations.map((conv) => {
                  const isOnline = useRealData && conv.buyerId ? buyerPresence.get(conv.buyerId) : conv.isOnline;
                  return (
                      ...
                      <div className="flex gap-4">
                        <div className="relative">
                          <Avatar className="h-12 w-12 border-2 border-white shadow-md ring-1 ring-orange-100">
                            <AvatarFallback className="bg-gradient-to-br from-orange-100 to-orange-200 text-orange-700 font-bold">{conv.buyerName.charAt(0)}</AvatarFallback>
                          </Avatar>
                          {isOnline ? (
                            ...
                            <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-gray-400 border-2 border-white rounded-full shadow-sm" />
                          )}
                        </div>
                        <div className="flex-1 min-w-0 py-0.5">
                          <div className="flex justify-between items-baseline mb-1">
                            ...
                            <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide ml-auto">{conv.lastMessageTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                          </div>
                          <p className={`text-sm truncate ${conv.unreadCount > 0 ? 'text-gray-900 font-bold' : 'text-gray-500 font-medium'}`}>{conv.lastMessage}</p>
                        </div>
                        {conv.unreadCount > 0 && <Badge className="bg-[var(--brand-primary)] shadow-lg shadow-orange-500/30 rounded-full h-5 min-w-[20px] flex items-center justify-center p-0.5 text-[10px] animate-pulse">{conv.unreadCount}</Badge>}
                      </div>
                    </div>
                  )
                })
            ...
            {activeConversation ? (
              <>
                <div className="bg-gradient-to-r from-[var(--brand-primary)] to-[var(--brand-primary-dark)] p-4 flex items-center gap-4 text-white shadow-lg shadow-orange-500/20 relative z-10 transition-all">
                  <Button variant="ghost" size="icon" className="text-white hover:bg-white/20 md:hidden rounded-full" onClick={() => setSelectedConversation(null)}><ChevronLeft className="h-6 w-6" /></Button>
                  <div className="relative">
                    <Avatar className="h-11 w-11 border-2 border-white/30 shadow-inner">
                      <AvatarFallback className="bg-white/20 text-white font-bold backdrop-blur-sm">{activeConversation.buyerName.charAt(0)}</AvatarFallback>
                    </Avatar>
                    {isHeaderOnline ? (
                      <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-teal-400 border-2 border-[var(--brand-primary)] rounded-full shadow-sm ring-1 ring-teal-500 animate-pulse" />
                    ) : (
                      <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-gray-400 border-2 border-[var(--brand-primary)] rounded-full shadow-sm" />
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="text-white hover:bg-white/20 rounded-full transition-colors"><Phone className="h-5 w-5" /></Button>
                    <Button variant="ghost" size="icon" className="text-white hover:bg-white/20 rounded-full transition-colors"><Video className="h-5 w-5" /></Button>
                    <Button variant="ghost" size="icon" className="text-white hover:bg-white/20 rounded-full transition-colors"><MoreVertical className="h-5 w-5" /></Button>
                  </div>
                </div>

                <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[var(--brand-wash)] scrollbar-hide">
                  {activeConversation.messages.map((msg) => {
                    // Render system messages as centered notification badges
                      ...
                      return (
                        ...
                        >
                          <div className="flex flex-col items-center gap-2 max-w-[70%]">
                            <div className="px-4 py-3 bg-gray-100 border border-gray-200 rounded-full shadow-sm flex items-center gap-2">
                              <div className="w-1.5 h-1.5 bg-gray-400 rounded-full" />
                              <p className="text-sm font-medium text-gray-700 text-center">{msg.text}</p>
                              <div className="w-1.5 h-1.5 bg-gray-400 rounded-full" />
                            </div>
                    ...
                    // Render regular chat messages with left/right bubble layout
                    return (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={msg.id} className={`flex ${msg.senderId === 'seller' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] flex flex-col ${msg.senderId === 'seller' ? 'items-end' : 'items-start'}`}>
                          ...
                          <span className="text-[10px] font-bold text-gray-400 mt-1.5 px-1 uppercase tracking-wider">{msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                        </div>
                      </motion.div>
                    );
                  })}
                ...
                <div className="p-4 bg-white border-t border-orange-100">
                  <form onSubmit={handleSendMessage} className="flex items-center gap-3 p-2 bg-gray-50/80 rounded-[20px] border border-gray-100 focus-within:border-orange-300 focus-within:bg-white focus-within:ring-4 focus-within:ring-orange-500/10 transition-all shadow-sm">
                    ...
                    <Button type="button" variant="ghost" size="icon" className="text-gray-400 hover:text-[var(--brand-primary)] hover:bg-orange-50 rounded-full h-10 w-10 transition-colors" onClick={() => fileInputRef.current?.click()}><ImageIcon className="w-5 h-5" /></Button>
