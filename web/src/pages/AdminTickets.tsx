...
export default function AdminTickets() {
  ...
  const { tickets, updateTicketStatus, loadTickets, sendAdminReply } =
    useSupportStore();

  const categories = useMemo(
    () => ["all", ...new Set(tickets.map((ticket) => ticket.category))],
    [tickets],
  );
  ...
  const filteredTickets = useMemo(
    ...
    [sortedTickets, activeTab, searchQuery, categoryFilter],
  );

  // Keep selected ticket in sync when store data updates (e.g., reply send)
  useEffect(() => {
    loadTickets().catch((err) => console.error("Load tickets failed:", err));
  }, []);
  ...
  const handleReplySend = async () => {
    ...
    await sendAdminReply(selectedTicket.id, replyText.trim());
    setReplyText("");
  };
          ...
          <div className="flex items-center justify-between mb-8 text-[var(--font-sans)]">
            <div>
              ...
              <p className="text-gray-600">
                Review and manage buyer support requests
              </p>
            </div>
            ...
            <CardContent className="p-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex gap-3 w-full lg:w-1/3">
                  ...
                  <select
                    className="flex-1 h-10 border border-gray-200 rounded-lg px-3 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)]/20"
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <Badge
                                    variant="outline"
                                    className={`text-[9px] uppercase tracking-tighter py-0 rounded-md border ${getStatusStyle(ticket.status)}`}
                                  >
                                    {ticket.status}
                                  </Badge>
                                </div>
                                ...
                                <p className="text-xs text-gray-500">
                                  {ticket.buyerName} • {ticket.category}
                                </p>
                              </div>
                              <div className="text-right flex flex-col items-end">
                                ...
                                <ChevronRight
                                  size={16}
                                  className="text-gray-300 group-hover:text-[var(--brand-primary)] transition-colors"
                                />
                              </div>
                            </motion.div>
                          ))
              ...
              <AnimatePresence mode="wait">
                {selectedTicket ? (
                      ...
                      <CardContent className="flex-1 p-6 space-y-6">
                        <div className="space-y-4">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <Badge
                              variant="outline"
                              className={`text-[11px] border ${getStatusStyle(selectedTicket.status)} whitespace-nowrap`}
                            >
                          ...
                          <div className="grid grid-cols-2 gap-3 text-xs text-gray-500">
                            <div>
                              <p className="font-semibold text-gray-800 text-[11px] uppercase tracking-wide mb-1">
                                Created
                              </p>
                              <p>
                                {new Date(
                          ...
                          <div className="pt-2">
                            ...
                            <div className="space-y-3 max-h-56 overflow-y-auto pr-1">
                              ...
                              selectedTicket.replies.length > 0 ? (
                                ...
                                <div className="text-xs text-gray-500 bg-white border border-dashed border-gray-200 rounded-lg p-3 flex items-center gap-2">
                  ...
                  <div className="min-h-[400px] flex flex-col items-center justify-center text-center p-8 bg-white rounded-xl border border-gray-100 border-dashed">
                    ...
                    <p className="text-sm text-gray-500">
                      Click on a ticket to view details and send a response.
                    </p>
                  </div>
                )}
              </AnimatePresence>
    ...
    <Card className="relative overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300">
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          ...
          <div
            className={`w-12 h-12 ${cardStyles[color].bg} rounded-xl flex items-center justify-center`}
          >
            <div className={`${cardStyles[color].text}`}>{icon}</div>
          </div>
        </div>
      </CardContent>
    </Card>
...
const TimelineItem = ({
  ...
  return (
    <div className={`flex ${isRight ? "justify-end" : "justify-start"}`}>
      <div className={`w-full ${isRight ? "text-right" : "text-left"}`}>
        <div
          className={`flex items-center gap-2 mb-1 ${isRight ? "justify-end" : ""}`}
        >
          <Badge
            variant="outline"
        ...
        <div className="bg-white border border-gray-100 rounded-lg p-3 shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
          {children}
        </div>
      </div>
    </div>
...
const StatusUpdateButton = ({
  ...
  const baseStyles =
    "w-full font-bold text-sm transition-all duration-200 h-11 rounded-lg";
