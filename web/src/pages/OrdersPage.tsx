...
export default function OrdersPage() {
  ...
  const isWithinReturnWindow = (order: any): boolean => {
    ...
    const deliveryDate = order.deliveryDate ? parseDate(order.deliveryDate) : new Date();
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const mappedOrders = (data || []).map(order => {
          ...
          const estimatedDelivery = order.estimated_delivery_date ? new Date(order.estimated_delivery_date) : new Date(createdAt.getTime() + 3 * 24 * 60 * 60 * 1000);
          ...
          return {
            ...
            deliveryDate: order.actual_delivery_date ? new Date(order.actual_delivery_date) : undefined,
            ...
            shippingAddress: {
              fullName: sa?.fullName || `${sa?.first_name || ''} ${sa?.last_name || ''}`.trim() || order.buyer_name,
              street: sa?.street || '',
              city: sa?.city || '',
                          ...
                          <div key={item.id} className="flex items-center justify-between gap-3 w-full">
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              ...
                              <div className="flex flex-wrap items-baseline gap-x-2">
                                ...
                                <span className="text-base font-bold text-[#FF6a00]">₱{item.price.toLocaleString()}</span>
                              </div>
                            </div>
                            <div className="text-base sm:text-lg font-bold text-gray-900 whitespace-nowrap pl-2">
                              Total: <span className="text-[#FF6a00]">₱{order.total.toLocaleString()}</span>
                            </div>
                          </div>
                        ))}
                      ...
                      {order.review && (
                        <div className="mt-2 pl-13 sm:pl-0">
                          <div className="flex items-center gap-3 mb-2">
                            {order.review.submittedAt && <span className="text-xs text-gray-500">Submitted on {new Date(order.review.submittedAt).toLocaleDateString()}</span>}
                            <span className="text-sm font-medium text-gray-500 flex items-center gap-1 ml-2">{order.review.rating}/5 <Star className="w-3.5 h-3.5 fill-yellow-400 text-yellow-400" /></span>
                          </div>
                          <div className="flex justify-between items-start gap-4">
                            <p className="text-sm text-gray-700 italic leading-relaxed bg-gray-50/50 p-2.5 rounded-lg w-full">"{order.review.comment}"</p>
                            {order.review.images && order.review.images.length > 0 && (
                              <div className="flex gap-1 shrink-0">
                                {order.review.images.map((img, idx) => (
                                  <div key={idx} className="w-12 h-12 rounded-lg border border-gray-200 overflow-hidden cursor-pointer hover:ring-2 hover:ring-[#FF6a00] transition-all" onClick={(e) => { e.stopPropagation(); setViewImage(img); }}>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex items-center justify-between cursor-pointer hover:bg-gray-50 rounded-lg -mx-2 px-2 py-1 transition-colors" onClick={() => navigate(`/order/${encodeURIComponent(order.id)}`)}>
                        <div className="flex items-center gap-3">
                          ...
                          <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium border ${getStatusColor(order.status)}`}>
                            {getStatusIcon(order.status)}
                            <span className="capitalize">{order.status === 'pending' ? 'Pending Payment' : order.status}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 text-sm text-gray-600">
                          <Calendar className="w-3.5 h-3.5" />
                            ...
                            <Button
                              onClick={async () => {
                                ...
                                const promises = order.items.map(item => {
                                  ...
                                  return addToCart(productInput as any, item.quantity, (item as any).selectedVariant);
                                });

                                // Wait for all items to be added before navigating
                                await Promise.all(promises);

                                navigate('/enhanced-cart', { state: { selectedItems: order.items.map(i => i.id) } });
                              }}
                              size="sm"
                            ...
                            <div className="flex flex-col items-end gap-3 text-right w-full sm:w-auto mt-4 sm:mt-0">
                              <div className="flex flex-col items-end gap-1">
                                <div className="flex items-center gap-1 bg-yellow-50 px-3 py-1.5 rounded-lg border border-yellow-100 mb-1">
                                  <span className="text-xs font-semibold text-yellow-700 uppercase tracking-wide mr-1">Your Rating</span>
                                  <div className="flex">
                                    {[1, 2, 3, 4, 5].map(star => (
                                      <Star
                                        key={star}
                              ...
                              {order.review?.images && order.review.images.length > 0 && (
                                <div className="flex gap-2 justify-end mt-1">
                                  {order.review.images.map((img, idx) => (
                                    <div key={idx} className="w-12 h-12 rounded-lg border border-gray-200 overflow-hidden">
                                      <img src={img} alt={`Review ${idx}`} className="w-full h-full object-cover" />
                                    </div>
                                  ))}
                                </div>
              ...
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h2 className="text-xl font-bold text-gray-900 mb-1">
                    Order #{selectedOrderData.id.includes('_') ? selectedOrderData.id.split('_')[1] : selectedOrderData.id}
                  </h2>
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-gray-600">Order #{viewReturnDetails.id.includes('_') ? viewReturnDetails.id.split('_')[1] : viewReturnDetails.id}</p>
                </div>
                <button
                  onClick={() => setViewReturnDetails(null)}
                  className="p-1 text-gray-400 hover:text-gray-600"
              ...
              <div className="space-y-6">
                ...
                {/* Return Request Info */}
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3">Return/Refund Information</h3>
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 space-y-3">
                    {viewReturnDetails.returnRequest ? (
                      <>
                        <div>
                          <span className="text-sm font-medium text-gray-700">Reason:</span>
                          <p className="text-gray-900 capitalize">{viewReturnDetails.returnRequest.reason.replace(/_/g, ' ')}</p>
                        </div>
                        <div>
                          <span className="text-sm font-medium text-gray-700">Solution:</span>
                          <p className="text-gray-900 capitalize">{viewReturnDetails.returnRequest.solution.replace(/_/g, ' ')}</p>
                        </div>
                        {viewReturnDetails.returnRequest.comments && (
                          <div>
                            <span className="text-sm font-medium text-gray-700">Comments:</span>
                            <p className="text-gray-900">{viewReturnDetails.returnRequest.comments}</p>
                          </div>
                        )}
                        <div>
                        ...
                        <div>
                          ...
                          <p className="text-gray-900">{new Date(viewReturnDetails.returnRequest.submittedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                        </div>
                        <div>
                      ...
                      <p className="text-gray-600">No return request data available</p>
                    )}
                  </div>
                </div>
              </div>
