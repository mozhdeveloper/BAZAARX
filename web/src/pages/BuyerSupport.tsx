...
const ServiceCard = ({ icon, label, onClick }: { icon: React.ReactNode, label: string, onClick?: () => void }) => (
    ...
    >
        ...
        <span className="text-[11px] font-medium text-center text-gray-600 group-hover:text-[#FF4500]">{label}</span>
    </button>
);

const ChatChip = ({ icon, label }: { icon: React.ReactNode, label: string }) => (
