/**
 * ╔═══════════════════════════════════════════════════════════════════════════╗
 * ║           BAZAARPH - COMPLETE DATA RESET & REPOPULATION V2                ║
...
async function cleanAllData() {
  ...
  const tables = [
    { name: 'order_items', condition: 'id' },
    { name: 'orders', condition: 'id' },
    { name: 'cart_items', condition: 'id' },
    { name: 'carts', condition: 'buyer_id' },
    { name: 'reviews', condition: 'id' },
    { name: 'product_images', condition: 'id' },
    { name: 'product_variants', condition: 'id' },
    { name: 'products', condition: 'id' },
    { name: 'seller_payout_accounts', condition: 'seller_id' },
    { name: 'seller_business_profiles', condition: 'seller_id' },
    { name: 'sellers', condition: 'id' },
    { name: 'buyers', condition: 'id' },
    { name: 'user_roles', condition: 'user_id' },
    { name: 'profiles', condition: 'id' },
    { name: 'vouchers', condition: 'id' },
    { name: 'categories', condition: 'id' },
  ];

  for (const table of tables) {
    const { error } = await supabase.from(table.name).delete().neq(table.condition, '00000000-0000-0000-0000-000000000000');
    console.log(`  ${error ? '❌' : '✅'} ${table.name}: ${error?.message || 'Cleared'}`);
  }
...
• Documentation`,
    ...
    },
    {
      ...
      product: {
        ...
        description: `Industry-leading noise canceling with eight microphones and Auto NC Optimizer. Designed to be supremely comfortable, with incredibly natural sound.
...
• Airplane adapter`,
      ...
      variants: [
        ...
        { variant_name: 'Midnight Blue', size: 'One Size', color: 'Midnight Blue', price: 22990, stock: 10, sku: 'XM5-BLU' },
      ],
      images: [
        { url: 'https://images.unsplash.com/photo-1546435770-a3e426bf472b?w=800', is_primary: true },
        { url: 'https://images.unsplash.com/photo-1583394838336-acd977736f90?w=800', is_primary: false },
      ],
    },
...
• Use shoe deodorizer`,
    ...
    },
    {
      ...
      product: {
        ...
        description: `A spacious and elegant tote bag crafted from premium vegan leather. Perfect for work, shopping, or everyday use.
...
• Handle drop: 25cm`,
      ...
      variants: [
        ...
        { variant_name: 'Nude', size: 'One Size', color: 'Nude', price: 2499, stock: 15, sku: 'TOTE-NDE' },
      ],
      images: [
        { url: 'https://images.unsplash.com/photo-1584917865442-de89df76afd3?w=800', is_primary: true },
        { url: 'https://images.unsplash.com/photo-1590874103328-eac38a683ce7?w=800', is_primary: false },
      ],
    },
...
• Tumble dry low`,
  ...
  for (const p of products) {
    ...
    if (!sellerId || !categoryId) {
      console.log(`  ❌ ${p.product.name}: Missing seller (${!!sellerId}) or category (${!!categoryId})`);
      continue;
    }

    console.log(`\n  📦 ${p.product.name}`);

    // Create product (QA APPROVED - using approval_status only)
...
async function createReviews(productMap: Record<string, string>, buyerMap: Record<string, string>) {
  ...
  const reviews = [
    ...
    // Sony headphones
    { sku: 'SONY-XM5', buyer: 'buyer1@bazaarph.com', rating: 5, title: 'Best headphones I\'ve owned', comment: 'Incredible sound quality and the noise cancellation is top tier. Worth every peso!' },
    ...
    { sku: 'BAG-TOTE-001', buyer: 'buyer3@bazaarph.com', rating: 5, title: 'Elegant and spacious', comment: 'Perfect size for work. Looks more expensive than it is!' },
...
async function main() {
  console.log('\n');
  console.log('╔═══════════════════════════════════════════════════════════════════════════╗');
  console.log('║           BAZAARPH - COMPLETE DATA RESET & REPOPULATION V2                ║');
  console.log('╚═══════════════════════════════════════════════════════════════════════════╝');
  ...
  const reviewCount = await createReviews(productMap, buyerMap);

  // Step 8: Create vouchers
  await createVouchers();

  // Calculate stock totals
  const { data: variants } = await supabase.from('product_variants').select('stock');
  ...
  ✅ Products:       ${Object.keys(productMap).length} (ALL QA APPROVED)
  ...
  console.log('    • seller1@bazaarph.com - TechHub Manila (Electronics)');
  console.log('    • seller2@bazaarph.com - Fashion Forward PH (Fashion)');
  console.log('    • seller3@bazaarph.com - Home & Living Co. (Home & Living)');
  console.log('');
  ...
  console.log('  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('  TechHub Manila (5 products):');
  console.log('    • iPhone 15 Pro Max');
  console.log('    • Samsung Galaxy S24 Ultra');
  console.log('    • Apple AirPods Pro (2nd Generation)');
  console.log('    • MacBook Air M3 (2024)');
  console.log('    • Sony WH-1000XM5 Wireless Headphones');
  console.log('');
  console.log('  Fashion Forward PH (5 products):');
  console.log('    • Premium Cotton Polo Shirt');
  console.log('    • Slim Fit Chino Pants');
  console.log('    • Floral Summer Dress');
  console.log('    • Canvas Sneakers Classic');
  console.log('    • Leather Tote Bag');
  console.log('');
  console.log('  Home & Living Co. (6 products):');
  console.log('    • Scandinavian Wooden Coffee Table');
  console.log('    • Premium Bedsheet Set (Queen)');
  console.log('    • Ceramic Plant Pot Set (3 pcs)');
  console.log('    • LED Smart Ceiling Light');
  console.log('    • Aromatherapy Essential Oil Diffuser');
  console.log('    • Organic Bamboo Bath Towel Set');
  console.log('');
}
