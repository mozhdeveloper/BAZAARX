...
Based on the current schema state and full code scan, the approved workflow is a compatibility-first phased migration.

Chosen approach:

1. Keep database model as-is (user_roles is source of truth, profiles.user_type remains absent).
2. Fix runtime blockers in web first, behind a feature flag.
3. Validate core user journeys before expanding to mobile.
4. Clean scripts/tests/types only after runtime is stable.

Why this is the most appropriate workflow here:

- It minimizes blast radius by touching only critical runtime paths first.
- It avoids reintroducing legacy schema assumptions that conflict with multi-role support.
- It enables controlled rollback at each phase without destructive DB actions.

Explicitly not selected:

- Reintroducing or relying on profiles.user_type.
- Big-bang migration across web + mobile + scripts in one release.

### Phase Priority (Least Disruptive)

1. Web runtime blockers only:
...
2. Web switch orchestration behind flag:
- Add/route through a shared role-switch service
- Gate with VITE_ROLE_SWITCH_V2
...
4. Cleanup phase:
- Scripts/tests/types that still mention legacy profiles.user_type

5. Post-stability hardening:
- RLS enablement and policy testing (separate rollout)

### Go/No-Go Gates

- Gate A: Web runtime and route guard tests pass.
- Gate B: Canary users can switch buyer/seller modes without incidents.
- Gate C: Mobile parity patch passes app-specific QA.
- Gate D: Cleanup complete with no runtime regressions.
